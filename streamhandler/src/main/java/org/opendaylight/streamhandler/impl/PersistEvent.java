...
public class PersistEvent {
	...
	public boolean sendDataToFlume(PersistEventInput input) throws IOException,
		...
		event = EventBuilder.withBody(eventBody,
				Charset.forName(StreamConstants.UTF_8));
			...
			if (!keyEvent.contains(StreamConstants.COLON)) {
				...
				for (String key : keyEvents) {
					if (subbody.get(key) instanceof JSONObject) {
						...
						header.put(keyEvent.replace(StreamConstants.COLON,
								StreamConstants.UNDERSCORE), (String) subbody
								.get(key));
		...
		if (eventFormat.equalsIgnoreCase(StreamConstants.STREAM)) {
			StreamCounterInfoCache streamCounterInfoCache = StreamCounterInfoCache
					.getInstance();
			streamCounterInfoCache.incrementCounter(header
					.get(StreamConstants.STREAM_STREAM_ID));
			SubscriberImpl.publishToURL(null,
					header.get(StreamConstants.STREAM_STREAM_ID), eventBody);
		}
		if (eventFormat.equalsIgnoreCase(StreamConstants.ALERT)) {
			StreamCounterInfoCache streamCounterInfoCache = StreamCounterInfoCache
					.getInstance();
			streamCounterInfoCache.incrementCounter(header
					.get(StreamConstants.ALERT_CONDITION_ID));
			SubscriberImpl.publishToURL(
					header.get(StreamConstants.ALERT_CONDITION_ID),
