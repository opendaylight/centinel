...
public class ConfigurationServiceImpl implements ConfigurationService, AutoCloseable {

    private static final Logger LOG = LoggerFactory.getLogger(ConfigurationServiceImpl.class);

    public static final InstanceIdentifier<ConfigurationRecord> configurationRecordId = InstanceIdentifier.builder(
            ConfigurationRecord.class).build();
    ...
    public Future<RpcResult<SetCentinelConfigurationsOutput>> setCentinelConfigurations(
        ...
        if (input.getGraylogIp().isEmpty() || input.getGraylogIp().trim().isEmpty() || input.getGraylogIp() == null) {
            ...
            return Futures.immediateFailedCheckedFuture(new TransactionCommitFailedException("invalid-input",
                    RpcResultBuilder.newError(ErrorType.APPLICATION, "invalid-input", "Graylog IP cannot be null")));
        }
        ...
        final SetCentinelConfigurationsOutputBuilder SetCentinelConfigurationsOutputBuilder = new SetCentinelConfigurationsOutputBuilder();

        SetCentinelConfigurationsOutputBuilder.setGraylogIp(graylogIp);
        SetCentinelConfigurationsOutputBuilder.setDrillIp(drillIp);
        SetCentinelConfigurationsOutputBuilder.setFlumeIp(flumeIp);
        SetCentinelConfigurationsOutputBuilder.setGraylogPort(graylogPort);
        SetCentinelConfigurationsOutputBuilder.setFlumePort(flumePort);
        SetCentinelConfigurationsOutputBuilder.setDrillPort(drillPort);
        SetCentinelConfigurationsOutputBuilder.setSyslogPort(syslogPort);
        try {
            tx.merge(LogicalDatastoreType.OPERATIONAL, configurationRecordId, new ConfigurationRecordBuilder()
                    .setGraylogIp(graylogIp).setDrillIp(drillIp).setFlumeIp(flumeIp).setFlumePort(flumePort)
                    .setDrillPort(drillPort).setGraylogPort(graylogPort).setSyslogPort(syslogPort).build(), true);
            tx.submit();

            notificationProvider.publish(new ConfigurationChangedBuilder().setGraylogIp(graylogIp)
                    .setDrillIp(drillIp).setFlumeIp(flumeIp).setGraylogPort(graylogPort)
                    .setFlumePort(flumePort).setDrillPort(drillPort).setSyslogPort(syslogPort).build());
            LOG.info("notification published");
            futureResult.set(RpcResultBuilder.<SetCentinelConfigurationsOutput> success(
                    SetCentinelConfigurationsOutputBuilder.build()).build());
        } catch (Exception e) {
